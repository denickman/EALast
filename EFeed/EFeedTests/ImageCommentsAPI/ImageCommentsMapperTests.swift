...
final class ImageCommentsMapperTests: XCTestCase {
    ...
    func test_map_throwsErrorOnNon2xxHTTPResponse() throws {
        ...
        try samples.forEach { code in
            ...
            XCTAssertThrowsError(try ImageCommentsMapper.map(data, from: response))
        }
    }
    ...
    func test_map_throwsErrorOn2xxHTTPResponseWithInvalidData() throws {
        ...
        try samples.forEach { code in
            let response = HTTPURLResponse(statusCode: code)
            XCTAssertThrowsError(try ImageCommentsMapper.map(invalidData, from: response))
        }
    }
    ...
    func test_map_deliversNoItemsOn2xxHTTPResponseWithEmptyData() throws {
        ...
        try samples.forEach { code in
            let response = HTTPURLResponse(statusCode: code)
            let result = try ImageCommentsMapper.map(emtpyData, from: response)
    ...
    func test_map_devliersItemsOn2xxHTTPResponseWithJSONItems() throws {
        ...
        try samples.forEach { code in
            let response = HTTPURLResponse(statusCode: code)
            let result = try ImageCommentsMapper.map(jsonData, from: response)
